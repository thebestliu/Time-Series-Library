...
class TimesBlock(nn.Module):
    def __init__(self, configs):
        ...
        #                        num_kernels=configs.num_kernels)
        # )
        self.c3d = C3D(configs.d_model, configs.d_ff)
    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            ...
            # Reshape for C3D
            out = out.reshape(B, length, N).permute(0, 2, 1).unsqueeze(3)  # [B, N, length, 1]
            out = out.repeat(1, 1, 1, period)  # Repeat to match period length [B,N,length,period]
            # Use C3D model
            out = self.c3d(out)  # Now out is [B, num_classes, N] if C3D has classification output

            # Reshape back to maintain B, N, length of period, period
            out = out.permute(0, 2, 1).reshape(B, N, length // period, period)
